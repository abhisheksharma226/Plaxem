...
import { Search, Bookmark, RotateCw, CheckCircle2 } from "lucide-react"
...
export function QuestionViewer({ questions, title, icon }: QuestionViewerProps) {
  const {
    completedQuestions,
  ...
  const filteredQuestions = useMemo(() => {
    return questions.filter((q) => {
      ...
      const matchesStatus =
        statusFilter === "all" ||
        (statusFilter === "solved" && isQuestionComplete(q.id)) ||
        (statusFilter === "unsolved" && !isQuestionComplete(q.id)) ||
        (statusFilter === "revision" && isInRevision(q.id))
      return matchesSearch && matchesDifficulty && matchesStatus
    })
  }, [questions, searchQuery, difficultyFilter, statusFilter, completedQuestions])
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Master your preparation with focused practice</p>
        </div>
      </div>
